...
export const PromHistogram = createParamDecorator(
  (data: IMetricArguments | string) => {
    ...
    if (!arg.name || arg.name.length === 0) {
      throw new Error(
        `PromHistogram need an argument, must be a fulfilled string or IPromHistogramDecoratorArg instance`,
      );
    }
